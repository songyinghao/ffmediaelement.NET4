namespace Unosquare.FFME.Windows.Sample.Kernel
{
    ...
    public class DelegateCommand : ICommand
    {
        ...
        {
            if (Thread.VolatileRead(ref IsExecuting) == 1) return;

            try
            {
                Interlocked.Exchange(ref IsExecuting, 1);
                await TaskEx.Run(() => { Application.Current.Dispatcher.BeginInvoke(DispatcherPriority.Normal, ExecuteAction, parameter); });
            }
            catch (Exception ex)
            {
