namespace Unosquare.FFME.Shared
{
    ...
    public unsafe class MediaInfo
    {
        ...
        internal MediaInfo(MediaContainer container)
        {
            ...
            Streams = new Dictionary<int, StreamInfo>(ExtractStreams(ic).ToDictionary(k => k.StreamIndex, v => v));
            ...
            BestStreams = new Dictionary<AVMediaType, StreamInfo>(FindBestStreams(ic, Streams));
        }
        ...
        /// </summary>
        public Dictionary<AVMediaType, StreamInfo> BestStreams { get; }
        ...
        private static List<StreamInfo> ExtractStreams(AVFormatContext* ic)
        {
            ...
            for (var i = 0; i < ic->nb_streams; i++)
            {
                ...
                var stream = new StreamInfo
                {
                    StreamId = s->id,
                    StreamIndex = s->index,
                    Metadata = new Dictionary<string, string>(FFDictionary.ToDictionary(s->metadata)),
        ...
        private static Dictionary<AVMediaType, StreamInfo> FindBestStreams(AVFormatContext* ic, Dictionary<int, StreamInfo> streams)
        {
        ...
        private static List<ChapterInfo> ExtractChapters(AVFormatContext* ic)
        {
            ...
            for (var i = 0; i < ic->nb_chapters; i++)
            {
                ...
                var chapter = new ChapterInfo
                {
                    ...
                    Index = i,
                    ChapterId = c->id,
                    Metadata = new Dictionary<string, string>(FFDictionary.ToDictionary(c->metadata))
                };
        ...
        private static List<ProgramInfo> ExtractPrograms(AVFormatContext* ic, Dictionary<int, StreamInfo> streams)
        {
            ...
            for (var i = 0; i < ic->nb_programs; i++)
            {
                ...
                {
                    Metadata = new Dictionary<string, string>(FFDictionary.ToDictionary(p->metadata)),
                    ProgramId = p->id,
